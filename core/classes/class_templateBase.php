...
class templateBase 
{
    ...
    {
        $plugin_vars = $this->pluginsControl->getVars();
        if (!count($plugin_vars)) return TRUE;

        foreach ($this->assigned_vars as $k => $v) {

            if (isset($plugin_vars[ $k.'_replace' ])) $this->assigned_vars[$k] = $plugin_vars[ $k.'_replace' ];
            else {
        ...
        foreach ($tmp as $k => $v) {
            if (!isset($this->assigned_vars[$k])) $this->assigned_vars[$k] = $tmp[ $k ];
        }
