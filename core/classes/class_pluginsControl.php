...
class pluginsControl 
{
    ...
    public function __call($method, $args) 
    {
        ...
        if (count($plugins)) {
            foreach ($plugins as $p) {
                if ($p == $not_found) continue;

                $tmp = $this->_installedPlugins[$p]->{$method}($args); 
                if ($tmp !== NULL) $this->_event_result[$method] = $tmp;
            }
        }
    ...
    private function loadUp() 
    {
        ...
        foreach ($this->_installedPlugins as $plugin){
            require $K->PLUGINS_DIR.$plugin.'/plugin.php';
    ...
    private function _checkEventCache( $event_name ) 
    {
        ...
        if (isset($this->_plugin_cache_array[ $event_name ])) return $this->_plugin_cache_array[ $event_name ];

        $this->_db2->query('SELECT plugin_name FROM plugins_cache WHERE event_name="'.$event_name.'"');
        while ($o = $this->_db2->fetch_object()) $data[] = $o->plugin_name;
