<?php
    ...
    $pikey = isset($_POST['ik']) ? (trim($db1->e($_POST['ik']))) : '';
    ...
    if (!$error && (empty($pikey) || strlen($pikey) != 15)) {
        $error = TRUE;
        $msgerror = 'Error.'; 
    }
    ...
    if ($error) {
        ...
        $pikey = $the_sanitaze->str_nohtml($pikey, 15);
        
        /******************************/

        $r = $db1->query("SELECT idproduct, ikey, linkproduct FROM products WHERE code='".$pcode."' LIMIT 1", FALSE);
